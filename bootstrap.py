...
def table_exists(name): return c.execute("SELECT 1 FROM sqlite_master WHERE type='table' AND name=?",(name,)).fetchone() is not None
def columns(name): return {r['name'] for r in c.execute(f'PRAGMA table_info({name})')}
if table_exists('trips'):
 old=columns('trips')
 for name,default in [('country',"''"),('region',"''"),('title',"''"),('companions',"''"),('trip_type',"''"),('status',"'완료'"),('lodging',"''"),('transport',"''"),('notes',"''")]:
  if name not in old: c.execute(f'ALTER TABLE trips ADD COLUMN {name} TEXT DEFAULT {default}')
 now=columns('trips')
 if 'destination' in now: c.execute("UPDATE trips SET title=COALESCE(NULLIF(title,''),destination), region=COALESCE(NULLIF(region,''),destination), country=COALESCE(NULLIF(country,''),destination)")
 if 'kind' in now: c.execute("UPDATE trips SET trip_type=COALESCE(NULLIF(trip_type,''),kind)")
 if 'category' in now: c.execute("UPDATE trips SET status=CASE WHEN category IN ('예정','검토 중','장기 계획') THEN category ELSE COALESCE(NULLIF(status,''),'완료') END")
else: c.execute('''CREATE TABLE trips(id INTEGER PRIMARY KEY,start_date TEXT,end_date TEXT,country TEXT,region TEXT,title TEXT,companions TEXT,trip_type TEXT,status TEXT,lodging TEXT,transport TEXT,notes TEXT)''')
...
for day in ('화','목'):
 ...
 if not exists: c.execute("INSERT INTO academy(day_of_week,start_time,end_time,academy,subject,location,notes,active) VALUES(?,?,?,?,?,?,?,1)",(day,'21:00','','화상영어','영어','',''))
c.commit(); c.close()
