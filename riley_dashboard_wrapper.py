...
DASH_CSS='''<style>
.rd-wrap{max-width:1180px;margin:auto}.rd-head{display:flex;justify-content:space-between;gap:10px;align-items:center;margin:14px 0 10px}.rd-actions{display:flex;gap:7px;flex-wrap:wrap}.rd-tabs{display:flex;gap:6px;margin:10px 0 14px;flex-wrap:wrap}.rd-tabs button{padding:8px 13px;border:1px solid #dfe6ee;border-radius:999px;color:#65758b;background:#fff;cursor:pointer;font:inherit}.rd-tabs button.on{background:#0f4c81;color:#fff;border-color:#0f4c81}.rd-kpis{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-bottom:14px}.rd-kpi{background:#fff;border:1px solid #e4e9f0;border-radius:14px;padding:14px}.rd-kpi b{display:block;font-size:25px;color:#0f4c81}.rd-kpi span{font-size:12px;color:#7a8798}.rd-grid{display:grid;grid-template-columns:1.4fr .8fr;gap:12px}.rd-card{background:#fff;border:1px solid #e4e9f0;border-radius:15px;padding:14px;margin-bottom:12px}.rd-row{display:grid;grid-template-columns:100px 1fr 52px;gap:10px;align-items:center;margin:9px 0}.rd-bar{height:9px;background:#eaf0f6;border-radius:99px;overflow:hidden}.rd-fill{height:100%;background:#0f4c81;border-radius:99px}.rd-book{padding:9px 0;border-bottom:1px solid #edf1f5}.rd-book:last-child{border-bottom:0}.rd-meta{font-size:12px;color:#7a8798;margin-top:3px}.rd-trend{overflow-x:auto}.rd-trend table{min-width:680px}.rd-lang{display:flex;height:10px;border-radius:99px;overflow:hidden;background:#eaf0f6;min-width:120px}.rd-lang .ko{background:#0f4c81}.rd-lang .en{background:#7ea7cc}.rd-level{font-weight:800;color:#0f4c81}.rd-panel{display:none}.rd-panel.on{display:block}@media(max-width:700px){.rd-kpis{grid-template-columns:repeat(2,1fr)}.rd-grid{grid-template-columns:1fr}.rd-row{grid-template-columns:78px 1fr 40px}.rd-head{align-items:flex-start}}
</style>'''
...
def _bars(rows,view):
 grouped=Counter(_period_key(r['read_date'],view) for r in rows)
 items=list(grouped.items())[:31 if view=='day' else 16 if view=='week' else 12]
...
def _monthly_trends(rows):
 ...
 for m in sorted(by.keys(),reverse=True)[:12]:
  ...
  sr=[_num(r.get('sr_score')) for r in rs if r.get('language')=='영어' and _num(r.get('sr_score')) is not None]; lx=[_num(r.get('lexile_score')) for r in rs if r.get('language')=='영어' and _num(r.get('lexile_score')) is not None]
  kop=ko/total*100 if total else 0; enp=en/total*100 if total else 0
  out+=f'<tr><td><b>{H(m)}</b></td><td>{total}권</td><td><div class="rd-lang"><span class="ko" style="width:{kop:.0f}%"></span><span class="en" style="width:{enp:.0f}%"></span></div><div class="rd-meta">한글 {ko} · 영어 {en}</div></td><td class="rd-level">{(f"{sum(sr)/len(sr):.1f}" if sr else "-")}</td><td class="rd-level">{(f"{sum(lx)/len(lx):.0f}L" if lx else "-")}</td></tr>'
...
def _dashboard_body(embedded=False):
 ...
 panels=''.join(f'<div id="rd-{v}" class="rd-panel {"on" if v=="week" else ""}"><h3>{label} 독서량</h3>{_bars(rows,v)}</div>' for v,label in [('day','일별'),('week','주별'),('month','월별')])
 ...
 script='''<script>function rdView(btn,v){var w=btn.closest('.rd-wrap');w.querySelectorAll('.rd-tabs button').forEach(function(x){x.classList.remove('on')});btn.classList.add('on');w.querySelectorAll('.rd-panel').forEach(function(x){x.classList.remove('on')});var p=w.querySelector('#rd-'+v);if(p)p.classList.add('on');}</script>'''
 return DASH_CSS+f'<div class="rd-wrap">{head}<div class="rd-tabs">{tabs}</div><div class="rd-kpis"><div class="rd-kpi"><b>{today_n}</b><span>오늘</span></div><div class="rd-kpi"><b>{week_n}</b><span>이번 주</span></div><div class="rd-kpi"><b>{month_n}</b><span>이번 달</span></div><div class="rd-kpi"><b>{len(rows)}</b><span>날짜 등록 누적</span></div></div><div class="rd-grid"><div><div class="rd-card">{panels}</div><div class="rd-card"><h3>최근 읽은 책</h3>{recent or "<div class=muted>데이터 없음</div>"}</div></div><div><div class="rd-card"><h3>언어 구성</h3><p><b>한글 {kor}권</b> · 영어 {eng}권</p></div>{levels}</div></div>{_monthly_trends(rows)}</div>'+script

@app.route('/riley/reading/dashboard')
def riley_reading_dashboard(): return page('지유 독서 대시보드',_dashboard_body(False))

import home_tasks_only_app as _main
_old=_main._reading_section

@app.route('/riley/reading/db')
def riley_reading_db(): return page('지유 독서 DB','<div class="toolbar"><h1>지유 독서 DB</h1><a class="btn s" href="/riley">지유 포탈</a></div>'+_old('지유','/riley')+_main.reading_edit_modal('/riley'))

def _portal_dashboard(child='지유',base='/riley'):
 if child=='지유' and base=='/riley': return _dashboard_body(True)
