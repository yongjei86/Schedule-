...
def _backfill_credit_event_dates():
    c=db()
...
def riley_credits_detail():
    ...
    c=db()
    rows=[dict(x) for x in c.execute("""select * from riley_credits
        where child=?
        order by coalesce(nullif(event_date,''),substr(created_at,1,10)) desc, created_at desc""",(child,)).fetchall()]
    c.close()

    maps={
        'workbook':{'day':{},'week':{},'month':{}},
        'reading':{'day':{},'week':{},'month':{}},
    }
    by_day={}
    for r in rows:
        ...
        maps[category]['week'][monday.isoformat()]=maps[category]['week'].get(monday.isoformat(),0)+delta
        maps[category]['month'][mk]=maps[category]['month'].get(mk,0)+delta
    ...
    def chart_panel(view,label,items,kind,active=False):
        ...
        bars=''
        for x,v in items:
            h=4 if v==0 else max(10,round(abs(v)/mx*150))
            bars+=(f'<div class="credit-bar-col" title="{H(x)} · {v:+d}">'
                   f'<div class="credit-bar-value">{v:+d}</div>'
                   f'<div class="credit-bar-track"><div class="credit-bar-fill {kind}" style="height:{h}px"></div></div>'
                   f'<div class="credit-bar-label">{H(x)}</div></div>')
        return (f'<div class="credit-chart-panel{" on" if active else ""}" data-credit-view="{view}">'
                f'<div class="credit-chart-note">{label}</div><div class="credit-bars">{bars}</div></div>')
    ...
    .credit-chart-head{display:flex;align-items:center;justify-content:space-between;gap:10px;flex-wrap:wrap;margin-bottom:12px}
    ...
    .credit-chart-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}
    .credit-chart-card{border:1px solid #e4e9f0;border-radius:13px;padding:12px;min-width:0}
    .credit-chart-card h3{margin:0 0 6px;font-size:15px}
    .credit-chart-panel{display:none}.credit-chart-panel.on{display:block}
    .credit-chart-note{font-size:11px;color:#8390a1;margin:2px 0 8px}
    .credit-bars{height:214px;display:flex;align-items:flex-end;gap:7px;overflow-x:auto;padding:8px 3px 6px;border-bottom:1px solid #edf1f5}
    .credit-bar-col{height:100%;min-width:36px;flex:1;display:flex;flex-direction:column;justify-content:flex-end;align-items:center}
    .credit-bar-value{font-size:9px;font-weight:800;color:#5f6f83;margin-bottom:4px;white-space:nowrap}
    .credit-bar-track{height:150px;width:21px;display:flex;align-items:flex-end;justify-content:center;background:#f1f4f8;border-radius:8px 8px 2px 2px;overflow:hidden}
    .credit-bar-fill{width:100%;border-radius:8px 8px 2px 2px;transition:height .18s ease}
    .credit-bar-fill.workbook{background:linear-gradient(180deg,#2f7fc1,#0f4c81)}
    .credit-bar-fill.reading{background:linear-gradient(180deg,#e9a65a,#d87832)}
    .credit-bar-label{font-size:9px;color:#7c8999;margin-top:6px;white-space:nowrap}
    .credit-kind{display:inline-block;border-radius:999px;padding:2px 7px;font-size:10px;font-weight:800;margin-right:4px}
    .credit-kind.workbook{background:#eaf3fb;color:#0f4c81}.credit-kind.reading{background:#fff1e5;color:#b96020}
    @media(max-width:800px){.credit-chart-grid{grid-template-columns:1fr}}
    @media(max-width:560px){.credit-chart-shell{padding:12px}.credit-tabs{width:100%}.credit-tab{flex:1}.credit-bars{gap:5px}.credit-bar-col{min-width:34px}.credit-bar-track{width:19px}}
    </style>'''
    ...
    function creditView(btn,view){
      ...
      document.querySelectorAll('.credit-chart-panel').forEach(function(x){
        x.classList.toggle('on',x.dataset.creditView===view);
      });
    }
    </script>'''

    body=(f'{credit_css}<div class="toolbar"><a class="btn s" href="/riley">← 지유 포탈</a></div>'
          f'<section class="credit-chart-shell"><div class="credit-chart-head"><div><h2>🪙 {H(child)} 크레딧 추이</h2>'
          f'<div class="feature-meta">버튼을 누른 시간이 아니라 문제집 예정일 · 독서일 기준</div></div>'
          f'<div class="credit-tabs">'
          f'<button type="button" class="credit-tab on" data-view="day" onclick="creditView(this,this.dataset.view)">일간</button>'
          f'<button type="button" class="credit-tab" data-view="week" onclick="creditView(this,this.dataset.view)">주간</button>'
          f'<button type="button" class="credit-tab" data-view="month" onclick="creditView(this,this.dataset.view)">월간</button>'
          f'</div></div><div class="credit-chart-grid">'
          f'{chart_card("workbook","📘 문제집 크레딧")}'
          f'{chart_card("reading","📚 독서 크레딧")}'
          ...
          f'<label style="display:flex;justify-content:space-between;align-items:center;gap:10px"><span>문제집 완료 시</span>'
          f'<span><input type="number" name="workbook" value="{wb_rate}" min="0" max="20" style="width:70px;padding:8px;border:1px solid #e4e9f0;border-radius:10px;text-align:center"> 개</span></label>'
          f'<label style="display:flex;justify-content:space-between;align-items:center;gap:10px"><span>책 1권 추가 시</span>'
          f'<span><input type="number" name="reading" value="{rd_rate}" min="0" max="20" style="width:70px;padding:8px;border:1px solid #e4e9f0;border-radius:10px;text-align:center"> 개</span></label>'
    ...
    for d in sorted(by_day.keys(),reverse=True):
        ...
        for it in info['items']:
            ...
            body+=(f'<div class="feature-meta" style="margin-top:4px"><span class="credit-kind {kind}">{kind_label}</span>'
                   f'{H(it["reason"] or "")} ({int(it["delta"]):+d}) · 기록 {H(created)}</div>')
        body+='</div>'
    body+='</section>'
...
,reason)
        ...
        if not event_date and category=='reading':
            ...
            if title:
                rr=c.execute("""select read_date from riley_reading
                                where child=? and title=? and read_date is not null and read_date!=''
                                order by abs(julianday(created_at)-julianday(?)) asc,id desc limit 1""",
                             (r['child'],title,r['created_at'])).fetchone()
