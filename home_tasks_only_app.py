...
def editable_trip_detail(trip_id):
    ...
    actions=(f'<div class="trip-actions"><a class="btn s" href="{back}">← 여행 목록</a>'
             f'<form method="post" action="/trip/{r["id"]}/status" class="status-form" style="display:inline-block;margin:0"><select name="status" class="status-select {scls}" style="width:auto" onchange="this.form.submit()">{stopts}</select></form>'
