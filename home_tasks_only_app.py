...
CSS+='''.future-card{display:flex;flex-direction:column;cursor:pointer}.future-card-detail{display:none;margin-top:8px;padding-top:8px;border-top:1px solid #edf1f5;cursor:default}.future-card-actions{display:flex;gap:6px;flex-wrap:wrap;margin-top:8px}.future-card-actions form{margin:0}.status-form{margin-top:0}.status-select{width:100%;border:1px solid #d7dfe8;border-radius:8px;padding:6px 8px;font-size:12px;font-weight:800;cursor:pointer;background:#eef2f6;color:#5c6b80}.status-select.planned{background:#e8f6ee;color:#1f7a4d;border-color:#bfe4cd}.status-select.review{background:#fff3e0;color:#b5680a;border-color:#f3d9ab}.status-select.longterm{background:#f1ecfb;color:#6a4fb0;border-color:#dccdf5}.status-select.done{background:#eef2f6;color:#5c6b80;border-color:#dfe6ee}'''
...
<div id="ed-triplink" style="margin-top:12px"></div></div></div>'''
...
def future_filtered():
    ...
    for r in rows:
        ...
        body+=(f'<div class="past-card future-card" onclick="tf(\'fc{r["id"]}\')">'
               ...
               f'<div class="future-card-detail" id="fc{r["id"]}" onclick="event.stopPropagation()">'
               ...
               f'</form>'
               f'<div class="future-card-actions"><a class="btn s" href="/trip/{r["id"]}">상세보기</a>'
               f'<button class="btn s" {trip_attrs(r)} onclick="et(this)">수정</button>'
...
def _refresh_google_cache():
    ...
    try:
        ...
        for raw in events:
            ...
            if not sd:
                continue
            google_rows.append((sd,ed,str(e.get('title') or ''),str(e.get('category') or ''),str(e.get('person') or ''),str(e.get('notes') or ''),'google',str(e.get('start_time') or ''),str(e.get('end_time') or '')))
        ...
        if google_rows:
            c.executemany('insert into home_event_cache(start_date,end_date,title,category,person,notes,source,start_time,end_time,synced_at) values(?,?,?,?,?,?,?,?,?,?)',[r+(now,) for r in google_rows])
...
def _cached_calendar_events(start, end):
    ...
    cached=[dict(x) for x in c.execute(
        'select start_date,end_date,title,category,person,notes,source,start_time,end_time from home_event_cache where start_date<=? and end_date>=? order by start_date,id',
        (end.isoformat(),start.isoformat())
    ).fetchall()]
