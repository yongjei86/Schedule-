...
CSS+='''.to-top{position:fixed;right:18px;bottom:18px;width:46px;height:46px;border-radius:50%;background:#0f4c81;color:#fff;border:0;font-size:20px;line-height:1;cursor:pointer;box-shadow:0 4px 14px #0f4c8155;display:none;align-items:center;justify-content:center;z-index:20}.to-top.show{display:flex}@media(max-width:560px){.to-top{width:42px;height:42px;right:14px;bottom:14px;font-size:18px}}'''
JS+='''(function(){var b=document.getElementById("toTop");if(!b)return;function u(){if(window.scrollY>320){b.classList.add("show")}else{b.classList.remove("show")}}window.addEventListener("scroll",u,{passive:true});u();b.addEventListener("click",function(){window.scrollTo({top:0,behavior:"smooth"})})})();'''
...
def page(title,body):return f'<!doctype html><html lang="ko"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{H(title)}</title><style>{CSS}</style></head><body>{nav()}<main class="wrap"><div class="hero"><h1>{H(title)}</h1></div>{body}</main><button class="to-top" id="toTop" aria-label="맨 위로">↑</button><script>{JS}</script></body></html>'
def mods():
 if not admin():return ''
